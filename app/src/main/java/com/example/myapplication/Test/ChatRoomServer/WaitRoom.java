...
public class WaitRoom
{
    ...
    private static final int MAX_USER = 200;
    ...
    public WaitRoom(){}
        ...
        public synchronized int addUser(String name, ServerThread client){
        ...
        public synchronized int joinRoom(String name, ServerThread client, int roomNumber, String password){
          Integer roomNum = new Integer(roomNumber);
          ChatRoom room = (ChatRoom) roomHash.get(roomNum);
          if(room.isRocked()){
              if(room.checkPassword(password)){
                  if(!room.addUser(name,client)){
                      return ERR_ROOMSFULL;
                  }
              } else {
                  return ERR_PASSWORD;
              }
          }else if (!room.addUser(name, client)){
        ...
        public synchronized boolean quitRoom(String name, int roomNumber, ServerThread client){
           boolean returnValue = false;
           Integer roomNum = new Integer(roomNumber);
           ChatRoom room = (ChatRoom) roomHash.get(roomNum);
           if(room.delUser(name)){
